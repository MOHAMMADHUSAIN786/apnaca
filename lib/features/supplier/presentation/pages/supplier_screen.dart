// lib/features/supplier/presentation/pages/supplier_screen.dart
// UPDATED: FAB guarded with canManageSuppliers permission
...
class SupplierScreenState extends State<SupplierScreen> {
  ...
  void refreshSuppliers() => _fetchSuppliers();
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildSupplierField(controller: nameController, label: "Supplier Name", icon: Icons.person,
                        validator: (v) => (v == null || v.trim().isEmpty) ? "Enter supplier name" : null),
                    SizedBox(height: 14.h),
                    _buildSupplierField(controller: phoneController, label: "Phone", icon: Icons.phone),
                    SizedBox(height: 14.h),
                    _buildSupplierField(controller: emailController, label: "Email", icon: Icons.email),
                    SizedBox(height: 14.h),
                    _buildSupplierField(controller: gstController, label: "GST Number", icon: Icons.receipt_long),
                    SizedBox(height: 14.h),
                    _buildSupplierField(controller: stateController, label: "State", icon: Icons.location_city),
                    SizedBox(height: 14.h),
                    _buildSupplierField(controller: addressController, label: "Address",
                        icon: Icons.location_on, maxLines: 3),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: ElevatedButton(
                            ...
                            child: Text("Save", style: TextStyle(color: app_colors.black)),
                          ),
                        ),
                      ],
                    ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // ── FAB — only show if canManageSuppliers ─────────────
      floatingActionButton: perm.canManageSuppliers
          ? Padding(
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(onPressed: _fetchSuppliers, child: const Text('Retry')),
          ],
        ),
      );
    }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16.h),
            Text('No suppliers found', style: TextStyle(fontSize: 16.sp, color: Colors.grey)),
          ],
        ),
      );
    }
