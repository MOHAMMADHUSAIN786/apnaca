// lib/features/supplier/presentation/widgets/app_supplier_design.dart
...
class AppSupplierDesign extends StatelessWidget {
  ...
  Future<void> _deleteSupplier(BuildContext context) async {
    // ← Permission check
    if (!PermissionService.instance.canManageSuppliers) {
      showPermissionDeniedSnackBar(context, 'Supplier Delete');
      return;
    }
      ...
      showDialog(
        ...
        builder: (context) => const AlertDialog(
          content: Row(children: [
            CircularProgressIndicator(), SizedBox(width: 20), Text('Deleting supplier...'),
          ]),
        ),
      );

      try {
        await AppDatabase.instance.deleteSupplier(supplier.id!);
        if (Navigator.of(context).canPop()) Navigator.of(context).pop();
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${supplier.name} deleted successfully')),
        );
        if (onSupplierDeleted != null) onSupplierDeleted!();
      } catch (e) {
        if (Navigator.of(context).canPop()) Navigator.of(context).pop();
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _editSupplier(BuildContext context) async {
    // ← Permission check
    if (!PermissionService.instance.canManageSuppliers) {
      showPermissionDeniedSnackBar(context, 'Supplier Edit');
      return;
    }
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Container(
                          ...
                          child: Icon(Icons.local_shipping, color: app_colors.black),
                        ),
                        SizedBox(width: 12.w),
                        Expanded(
                    ...
                    _buildSupField(controller: nameController, label: "Supplier Name", icon: Icons.person,
                        validator: (v) => (v == null || v.trim().isEmpty) ? "Enter supplier name" : null),
                    SizedBox(height: 14.h),
                    _buildSupField(controller: phoneController, label: "Mobile Number", icon: Icons.phone),
                    SizedBox(height: 14.h),
                    _buildSupField(controller: emailController, label: "Email", icon: Icons.email_outlined),
                    SizedBox(height: 14.h),
                    _buildSupField(controller: gstController, label: "GST Number", icon: Icons.receipt_long),
                    SizedBox(height: 14.h),
                    _buildSupField(controller: stateController, label: "State", icon: Icons.location_city),
                    SizedBox(height: 14.h),
                    _buildSupField(controller: addressController, label: "Address",
                      ...
                      children: [
                        ...
                        Expanded(
                          child: ElevatedButton(
                            style: ElevatedButton.styleFrom(backgroundColor: app_colors.table_header_bg),
                            onPressed: () async {
                              ...
                              try {
                                ...
                                await AppDatabase.instance.updateSupplier(updatedSupplier);
                                Navigator.pop(context);
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text("Supplier updated successfully")),
                                );
                                if (onSupplierUpdated != null) onSupplierUpdated!();
                              } catch (e) {
                                ScaffoldMessenger.of(context).showSnackBar(
                            ...
                            child: Text("Update", style: TextStyle(color: app_colors.black)),
                          ),
                        ),
                      ],
                    ),
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.local_shipping, size: 20, color: app_colors.black),
            ),
            SizedBox(width: 12.w),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 4.h),
                  Text(
                    supplier.phone != null ? "📞 ${supplier.phone}" : (supplier.email ?? "No contact"),
            ...
            PopupMenuButton<String>(
              ...
              onSelected: (value) {
                if (value == 'edit') _editSupplier(context);
                else if (value == 'delete') _deleteSupplier(context);
              },
              itemBuilder: (context) => [
                if (perm.canManageSuppliers)
                  const PopupMenuItem(
                    value: 'edit',
